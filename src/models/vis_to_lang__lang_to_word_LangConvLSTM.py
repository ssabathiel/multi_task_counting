...
class LangConvLSTMCell(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, env,lr):
        ...
        # FC-LSTM:
        self.Gates_lang = nn.Linear(2*(n_count_words+1)+self.task_vector_length + self.vis_representation_size, 4*(n_count_words+1) )
        ...
        self.fc3_lang = nn.Linear(n_count_words+1, self.lang_representation_size)
    ...
    def forward(self, input_, prev_state, input_lang, prev_state_lang, env_task_list):
        ...
        stacked_inputs_lang = torch.cat((input_lang, prev_hidden_lang, env_task_list, output_repr), 1).view(-1, 2*(self.n_count_words+1)+self.task_vector_length + self.vis_representation_size)*10
