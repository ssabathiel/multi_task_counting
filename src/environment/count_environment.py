...
class CountEnv():
    ...
    def reset(self):
        ...
        #    self.quant_n = 0
            ...
            self.task_n = 4
            self.object_n = self.add_n
            self.quant_n = self.n_squares
            self.hand_is_visible = False
            self.square_is_visible = False
            self.max_time = self.n_squares*2 + self.add_n
            self.aimed_count_list = []
            for i in range(self.add_n):
              self.aimed_count_list.append(str(self.n_squares+1+i) )
    ...
    def update(self, action):
      ...
      if(self.task == "recite_n" or self.task == "recite_n_inverse" or self.task == "count_on"):
