...
def train_model(task_list=["touch_all_objects"], n_squares_ = 1, num_epochs = 2000, episode = 0, run_n = 0, model=None):
    ...
    for epoch in range(0, max_epoch):
          ...
          input_lang = torch.zeros( (batch_size, env.n_words) ).view(batch_size, env.n_words)
          ...
          for t in range(0, len(env_img_list)):
              ...
              #print("input_lang = env_action_list[t].shape: ", env_action_list[t].shape)
              #print("input_lang = env_action_list[t].shape: ", env_action_list[t][:,:-2].shape)
              #input_lang = env_action_list[t][-env.n_words:]
              input_lang = env_action_list[t][:,-env.n_words-1:-1]
